"""Django to unified Worker Mojtaba runtime bridge."""
...
class AgentBridge:
    def __init__(self, worker_base_url: str | None = None, token: str | None = None):
        self.worker_base_url = (worker_base_url or os.getenv("WORKER_MOJTABA_URL", "http://127.0.0.1:8787")).rstrip("/")
        self.token = token or os.getenv("WORKER_API_TOKEN", "")

    def execute_goal(self, goal: str, *, context: dict[str, Any] | None = None, approved: bool = False) -> dict[str, Any]:
        if not self.token:
            return {"status": "worker_auth_not_configured"}
        try:
            response = httpx.post(
                f"{self.worker_base_url}/v1/tasks",
                json={"request": goal, "context": context or {}, "approved": approved},
