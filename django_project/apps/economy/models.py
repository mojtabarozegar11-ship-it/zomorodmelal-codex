...
class SignalProduct(models.Model):
    ...
    disclaimer = models.TextField(
        default="Signals are informational and do not guarantee profit."
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class VirtualAssetProject(models.Model):
    ...
    ASSET_CLASSES = [
        ...
        ("tokenized_right", "Tokenized right"),
    ]
    name = models.CharField(max_length=200, unique=True)
    symbol = models.CharField(max_length=30, unique=True)
    asset_class = models.CharField(
        max_length=40, choices=ASSET_CLASSES, default="utility"
    )
    status = models.CharField(max_length=30, choices=STATUS, default="idea")
    concept = models.TextField()
    economic_model = models.TextField(blank=True)
    behavioral_model = models.TextField(blank=True)
    target_jurisdictions = models.TextField(
        blank=True,
        help_text="Jurisdictions considered for legal review.",
    )
    ...
    owner_approved = models.BooleanField(default=False)
    issuance_approved = models.BooleanField(default=False)
...
class EconomicWorkItem(models.Model):
    ...
    title = models.CharField(max_length=250)
    domain = models.CharField(max_length=30, choices=DOMAINS)
