...
from .models import Asset, MarketDataSnapshot, OrderIntent, Portfolio, SignalProduct, SignalPurchase

def dashboard(request):
    return render(request, "economy/dashboard.html", {"assets": Asset.objects.filter(active=True).count(), "portfolios": Portfolio.objects.filter(active=True).count(), "orders": OrderIntent.objects.count(), "market_points": MarketDataSnapshot.objects.count(), "signals": SignalProduct.objects.filter(active=True).count(), "agents": EconomicMasterAgent().capabilities()})

def health(request):
    return JsonResponse({"economic_agent": "ready", "paper_trading": True, "real_execution": False, "owner_approval_required": True, "signal_sales": True, "payment_gateway": "sandbox"})
