...
def get_idempotent_response(*, key, scope, actor):
    return IdempotencyRecord.objects.filter(key=key, scope=scope, actor=actor).first()

@transaction.atomic
def save_idempotent_response(*, key, scope, actor, response_code, response_payload):
    try:
        return IdempotencyRecord.objects.create(
            key=key, scope=scope, actor=actor,
            response_code=response_code, response_payload=response_payload,
        )
    except IntegrityError:
        return IdempotencyRecord.objects.get(key=key, scope=scope, actor=actor)
