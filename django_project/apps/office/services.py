...
def audit(actor, action, obj, company=None, metadata=None):
    return AuditLog.objects.create(
        actor=actor, company=company, action=action,
        object_type=obj.__class__.__name__, object_id=str(obj.pk), metadata=metadata or {},
    )
...
def accessible_companies(user):
    ...
    return Company.objects.filter(
        active=True, delegations__user=user, delegations__active=True,
...
def post_journal(*, company, journal_no, entry_date, lines, actor=None, description="", owner_approved=False):
    """Atomically validate and post a balanced double-entry journal."""
    ...
    accounts = [item["account"] for item in lines]
    if any(account.company_id != company.id or not account.active for account in accounts):
    ...
    for item in lines:
        ...
        total_debit += debit
        total_credit += credit
        normalized.append((item["account"], item.get("description", ""), debit, credit))

    if total_debit != total_credit:
        raise ValidationError("Total debit and credit must be equal.")

    journal = Journal.objects.create(
        company=company, journal_no=journal_no, entry_date=entry_date,
    ...
    JournalLine.objects.bulk_create([
        JournalLine(
            journal=journal, line_no=index, account=account,
            description=description_text, debit=debit, credit=credit,
        )
        for index, (account, description_text, debit, credit) in enumerate(normalized, start=1)
    ])
    journal.status = "posted"
    journal.posted_at = timezone.now()
    journal.save(update_fields=["status", "posted_at"])
    record_audit(
        actor=actor, action="journal_posted", scope="office.accounting", obj=journal,
