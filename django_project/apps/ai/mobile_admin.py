...
def mobile_manifest(request):
    return JsonResponse({
        ...
        "icons": [
            {"src": "/mobile-admin/icon.svg", "sizes": "any", "type": "image/svg+xml", "purpose": "any maskable"}
        ],
    })
...
def mobile_service_worker(request):
    script = """
const CACHE = "zomorod-admin-v1";
self.addEventListener("install", event => {
  event.waitUntil(caches.open(CACHE).then(cache => cache.add("/mobile-admin/")));
  self.skipWaiting();
});
...
<rect width="192" height="192" rx="40" fill="#0f5132"/>
<circle cx="96" cy="96" r="62" fill="#fff"/>
<path d="M96 45c-28 22-45 39-45 65 0 25 20 45 45 45s45-20 45-45c0-26-17-43-45-65z" fill="#198754"/>
<path d="M96 73c-11 12-18 22-18 35 0 10 8 18 18 18s18-8 18-18c0-13-7-23-18-35z" fill="#d4af37"/>
</svg>"""
