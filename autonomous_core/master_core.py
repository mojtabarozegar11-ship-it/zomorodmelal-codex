...
class MasterCore:
    ...
    GENERATIONS = {
        1: "observation", 2: "diagnosis", 3: "goal_prioritization", 4: "agent_delegation",
        5: "planning", 6: "sandbox_execution", 7: "testing", 8: "self_repair",
        9: "verification", 10: "learning", 11: "evolution", 12: "continuous_goals",
        13: "multi_agent_coordination", 14: "resilience_and_rollback", 15: "closed_loop_master_agent",
    }
    GOAL_ACCESS_RULES = {
    ...
    REQUIRED_CAPABILITIES = {
        "planning", "research", "memory", "knowledge_management", "evaluation", "sandbox_execution",
        "automated_testing", "rollback", "owner_approval", "execution_control", "orchestration",
        "agent_generation", "evolution",
    }
    ...
    def _defaults(self):
        return {"version": self.VERSION, "cycles": 0, "generation": 1, "phase": "observe", "goals": [],
                "agents": [], "capabilities": [], "proposals": [], "access_requests": [], "activation_checks": [],
                "mission_history": [], "learning": [], "failures": [], "last_cycle": None, "last_decision": None, "last_result": None}
    ...
    def set_goal(self, goal):
        ...
        item = {"id": max((int(x.get("id", 0)) for x in self.state["goals"]), default=0) + 1, "text": goal,
    ...
    def discover_capabilities(self):
        capabilities = set()
        mapping = {"research": "research", "agent_factory": "agent_generation", "factory": "agent_generation", "sandbox": "sandbox_execution",
                   "testing": "automated_testing", "evolution": "evolution", "versioning": "rollback", "approval": "owner_approval",
                   "execution": "execution_control", "orchestrator": "orchestration", "access": "access_management", "knowledge": "knowledge_management",
    ...
    def design_agents(self, missing):
        templates = {"planning": ("StrategicPlannerAgent", "تحلیل هدف و طراحی برنامه"), "research": ("ResearchAgent", "تحقیق و جمع‌آوری دانش"),
                     "sandbox_execution": ("SandboxAgent", "ساخت و آزمایش امن"), "automated_testing": ("TestingAgent", "تست و تشخیص شکست"),
                     "agent_generation": ("AgentFactoryAgent", "طراحی و تولید زیرایجنت"), "evolution": ("EvolutionAgent", "طراحی نسل جدید"),
    ...
    def check_activation(self, request_id, capability, scope="minimum_required"):
        ...
        self.state["activation_checks"].append({**decision, "checked_at": datetime.now(timezone.utc).isoformat()}); self._save_state(); return decision
    ...
    def plan_mission(self, mission):
        return {"mission_id": mission["id"], "steps": ["prepare_sandbox", "delegate", "execute_safe_actions", "test", "verify", "record_learning"], "safe_execution": True, "real_world_execution": False, "owner_approval_required": True}

    def _learn(self, mission, success, evidence=None):
        event = {"timestamp": datetime.now(timezone.utc).isoformat(), "mission": mission["id"], "success": bool(success), "generation": self.state["generation"], "evidence": evidence or {}}
    ...
    def run_cycle(self, goal=None, test_passed=None):
        ...
        missing = self.analyze_gaps(); designs = self.design_agents(missing)
