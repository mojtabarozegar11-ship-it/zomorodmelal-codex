...
class ChangePackageExecutor:
    ...
    def _safe_relative(self, relative: str) -> Path:
        raw = Path(relative)
        if raw.is_absolute() or ".." in raw.parts or not raw.name: raise ValueError("package path must be relative and contained")
        if raw.parts[0] in self.PROTECTED: raise ValueError("protected project path cannot be promoted")
        target = (self.root / raw).resolve()
        if self.root not in target.parents: raise ValueError("package path escapes project")
        return target

    def _verify_package(self, package_id: str) -> Dict[str, Any]:
        package = self.packages.get(package_id)
        if not self.packages.verify_integrity(package): raise ValueError("change package integrity check failed")
        if package.get("status") != "awaiting_owner_approval": raise PermissionError("change package is not awaiting owner approval")
        if not package.get("owner_approval_required", True): raise PermissionError("owner approval boundary is missing")
        return package

    def execute(self, package_id: str, request_id: int) -> Dict[str, Any]:
        request = self.approval.get(request_id)
        if not request or not self.approval.is_approved(request_id): return {"success": False, "status": "approval_required", "request_id": request_id}
        package = self._verify_package(package_id)
        metadata = request.get("metadata") or {}
        if metadata.get("package_id") != package_id: return {"success": False, "status": "package_binding_mismatch", "request_id": request_id}
        if metadata.get("package_sha256") != package.get("package_sha256"): return {"success": False, "status": "package_hash_binding_mismatch", "request_id": request_id}
        sandbox_rel = str(package.get("sandbox_rel") or "")
        if not sandbox_rel: return {"success": False, "status": "sandbox_source_missing", "request_id": request_id}
        sandbox = (self.root / sandbox_rel).resolve()
        if self.root not in sandbox.parents or not sandbox.is_dir(): return {"success": False, "status": "sandbox_source_invalid", "request_id": request_id}
        verified=[]
